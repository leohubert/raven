...
src/gen/         generated by `task proto` from proto/ - never hand-edited
native/          raven_native.m - the ObjC shim, compiled by scripts/build-native.ts
server/web/      the dashboard - React 19 + Tailwind v4, bundled from memory at server boot
```
...
A theme **pushed at runtime** by the server has no such folder - the bundle is signed and
read-only - so it cannot go through `views://` at all. Its bytes travel base64 over the
overlay RPC (`pushedTheme`), become blob URLs in `src/views/overlay/pushed.ts`, and are
resolved by `rel` at the point each URL is used. Resolution happens there, rather than by
rewriting the received `Theme` once, because every later burst arrives from the main
process still carrying the asset's original `pushed://` URL.
...
## Remote control — Task 0 spike result (macOS 26.6.2, arm64, Bun 1.4.2)

| # | Unknown | Result |
|---|---|---|
| 0 | Connect-ES bidi streaming over Bun's `node:http2` server | **PASS** — `bun scripts/spike-bidi.ts` printed `received 3`; both directions flowed over h2c on `127.0.0.1:8787` and the stream survived the 2s idle gap with no crash |

`@connectrpc/connect-node@2.2.0`'s `connectNodeAdapter` served `RavenControl.Connect`
(`stream ClientToServer -> stream ServerToClient`) directly off `http2.createServer`, and
`createGrpcTransport` + `createClient` from the same package drove it as a client in the same
process. No fallback needed: the plan continues to Task 1 with gRPC/Connect bidi streaming as
the transport, unchanged.

## Dashboard React/Tailwind — Task 0 spike result (macOS 26.6.2, arm64, Bun 1.4.2)

| # | Unknown | Result |
|---|---|---|
| 1 | `bun-plugin-tailwind@0.1.2` compiling `tailwindcss@4.3.3` syntax via `Bun.build` | **PASS** — `bun scripts/spike-tailwind.ts` generated real utility CSS (`grid-template-columns: repeat(7, 1fr)`, `.bg-bg { background-color: var(--color-bg) }`) for a scratch `@theme` + arbitrary-value class, not a passthrough |
| 2 | `tailwindcss@4.3.3`'s programmatic `compile()` as an in-process fallback | **PASS** — same assertions passed against `compile(css, { loadStylesheet }).build(candidates)`, run directly against the `tailwindcss` package with no plugin |

Both mechanisms compile Tailwind v4.3 syntax under Bun 1.4.2. `bun-plugin-tailwind@0.1.2`
bundles its own pinned Tailwind engine (`v4.1.14`, per the generated CSS's own banner comment)
rather than using the installed `tailwindcss@4.3.3` — the version-gap risk this spike was
scoped to check — but that older engine still compiled the 4.3-style `@theme` token and
arbitrary-value syntax used here correctly, so the gap did not surface a real incompatibility
in this test. The programmatic `compile()` export from `tailwindcss` itself has no built-in
module resolution: it throws `No \`loadStylesheet\` function provided` until one is supplied,
so a caller must resolve `@import "tailwindcss"` itself (`compile()` follows the "style"
package export condition Bun's own resolver doesn't know, so `tailwindcss/package.json`'s
directory + `index.css` stands in for it). Recommendation: use the programmatic `compile()`
path in Task 4 — it is symmetric with `bundleMain` (an in-process function returning a string,
version-locked to whatever `tailwindcss` is installed, no bundled second copy of the engine to
drift) — see `bundleCss` in `server/web/static.ts` for the working shape.
